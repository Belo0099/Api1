-- ╔══════════════════════════════════════════════════════════════════╗
-- ║           ASTRAX HUB  •  VIP PREMIUM EDITION  •  v2.0          ║
-- ║         Ultra-Modern UI Library for Roblox Exploiting           ║
-- ╚══════════════════════════════════════════════════════════════════╝
...
-- ══════════════════════════════════════════════════════════════════
--  PALETA PREMIUM  (fundo fosco escuro + vermelho vibrante/neon)
-- ══════════════════════════════════════════════════════════════════
local MyLibrary = {
    Themes = {
        Main = {
            -- Gradiente lateral: preto → vermelho profundo → vermelho vivo
            ...
            ["Color Hub 2"]      = Color3.fromRGB(12, 12, 14),   -- fundo fosco profundo
            ["Color Panel"]      = Color3.fromRGB(18, 18, 21),   -- painel levemente mais claro
            ["Color Stroke"]     = Color3.fromRGB(210, 15, 15),  -- borda neon vermelha
            ["Color Theme"]      = Color3.fromRGB(210, 15, 15),  -- destaque vermelho
            ["Color ThemeHover"] = Color3.fromRGB(240, 40, 40),  -- hover mais brilhante
            ["Color Text"]       = Color3.fromRGB(245, 245, 245),
            ["Color Dark Text"]  = Color3.fromRGB(140, 140, 150),
            ["Color Separator"]  = Color3.fromRGB(40, 40, 48),   -- linha divisória sutil
            ["Color Toggle Off"] = Color3.fromRGB(45, 45, 55),   -- trilho toggle desligado
            ["Color Card"]       = Color3.fromRGB(22, 22, 26),   -- fundo do card/opção
        },
    },
    ...
    Icons = (function()
        return {
            ...
            -- extras úteis para Anti-Tools / utilidades
...
do  -- destruir instância duplicada
...
-- Drag genérico (funciona em Frame e ImageButton)
...
-- ─── Frame de opção (título + descrição + holder direito) ────────
...
function MyLibrary:MakeWindow(Configs)
    ...
    -- ─── Janela principal ───────────────────────────────────────
    -- Começa invisível → anima a abertura
    ...
    -- Sombra (drop shadow simulada com frame maior por baixo)
    ...
    -- ── PERFIL ──────────────────────────────────────────────
    --  Altura total do cabeçalho = 78px
    local PROFILE_H = 78
    ...
    -- Avatar — moldura externa (borda colorida)
    ...
    local AvatarBorder = InsertTheme(Create("Frame", ProfileFrame, {
        ...
        -- centralizado verticalmente no ProfileFrame
        Position         = UDim2.new(0, 10, 0.5, 0),
        AnchorPoint      = Vector2.new(0, 0.5),
    ...
    -- Avatar — imagem interna (2px de inset)
    ...
    -- Badge VIP (pequena pill vermelha)
    ...
    -- ── Linha separadora abaixo do perfil ───────────────────
    ...
    -- ══════════════════════════════════════════════════════════
    --  ÁREA DE CONTEÚDO (direita da sidebar)
    -- ══════════════════════════════════════════════════════════
    ...
    -- ══════════════════════════════════════════════════════════
    --  BOTÕES DE JANELA — Fechar (X) e Minimizar (-)
    --  Posicionados no canto SUPERIOR DIREITO da janela
    -- ══════════════════════════════════════════════════════════
    ...
    local MinimizeButton = WinBtn(
        "rbxassetid://10734896206",          -- ícone minus
        Color3.fromRGB(55, 55, 65), 1)
    local CloseButton = WinBtn(
        "rbxassetid://10747384394",          -- ícone X
        Color3.fromRGB(190, 20, 20), 2)
    ...
    function Window:MakeTab(paste, Configs)
        ...
        -- Indicador lateral (linha vermelha)
        ...
        function Tab:Visible(b)
            ...
            elseif Container.Parent then Container.Parent = ContentContainer end
        end
        function Tab:Destroy() TabBtn:Destroy(); Container:Destroy() end

        -- ── SECTION ─────────────────────────────────────────
        function Tab:AddSection(cfg)
            local name = type(cfg)=="string" and cfg
                      or cfg[1] or cfg.Name or cfg.Title or "Section"
        ...
        function Tab:AddToggle(cfg)
            ...
            -- Tamanho do holder: deixa 50px para o switch
            ...
            local Track = InsertTheme(Create("Frame", fb, {
                ...
                BorderSizePixel  = 0,
                ZIndex           = 3,
            }), "Stroke")   -- usamos "Stroke" para não mudar a cor com o tema Frame
            Make("Corner", Track, UDim.new(1, 0))

            -- Knob (bolinha deslizante)
            ...
            -- Brilho interno do knob quando ON
            ...
            local function ApplyState(val, instant)
                ...
                if val then
                    -- ON: trilho vermelho, knob vai para direita
                    Tween(Track, "BackgroundColor3", Theme["Color Theme"], t)
                    Tween(Knob,  "Position",
                        UDim2.new(1, -(KNOB_S+3), 0.5, 0), t)
                    ...
                    -- OFF: trilho cinza, knob vai para esquerda
        ...
        function Tab:AddDropdown(cfg)
            ...
            -- Anti-click overlay + frame flutuante
        ...
        function Tab:AddTextBox(cfg)
            ...
            local placeholder = cfg[5] or cfg.PlaceholderText or "Type here..."
        ...
        -- ── ANTI TOOLS ──────────────────────────────────────
        -- Remove ferramentas do Backpack do jogador (Anti-Tool)
        function Tab:AddAntiTools(cfg)
            local name    = cfg and (cfg[1] or cfg.Name  or cfg.Title) or "Anti Tools"
            local desc    = cfg and (cfg.Desc or cfg.Description) or "Remove todas as ferramentas"
            ...
            -- Reutilizamos AddToggle internamente
            local tog = Tab:AddToggle({
                name, default, cb[1], flag,
                Desc = desc,
            })

            -- Lógica real de Anti-Tool
            local antiConn
            Connection.FlagsChanged:Connect(function(f, v)
                if f ~= flag then return end
                if v then
                    -- Limpar backpack agora
                    local bp = Player:FindFirstChildOfClass("Backpack")
                    if bp then for _, t in ipairs(bp:GetChildren()) do t:Destroy() end end
                    ...
                    antiConn = RunService.Heartbeat:Connect(function()
                        local b2 = Player:FindFirstChildOfClass("Backpack")
                        if b2 then for _, t in ipairs(b2:GetChildren()) do t:Destroy() end end
            ...
            return tog  -- retorna a API do Toggle para controle externo
        end
...
-- ══════════════════════════════════════════════════════════════════
--  SET THEME  (mantido para compatibilidade)
-- ══════════════════════════════════════════════════════════════════
function MyLibrary:SetScale(newScale)
    local s = ViewportSize.Y / math.clamp(newScale, 300, 2000)
    UIScale = s
