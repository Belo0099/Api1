...
function MyLibrary:MakeWindow(Configs)
    ...
    local MainFrame = InsertTheme(Create("ImageButton", ScreenGui, {
        ...
        BorderSizePixel    = 0,
        Name               = "AstraxHub",
        -- ✅ CRÍTICO: ClipsDescendants = false para a sombra externa aparecer
    ...
    local Shadow = Create("Frame", ScreenGui, {
        Size             = UDim2.fromOffset(SX + 30, SY + 30),
    ...
    -- ═══════════════════════════════════════════════════════════
    --  TOPBAR — barra superior unificada (título + avatar + botões)
    --  FIX: era um ProfileFrame dentro da Sidebar causando
    --       expansão horizontal. Agora é um Frame independente
    --       no topo do MainFrame, com largura = 100% da janela.
    -- ═══════════════════════════════════════════════════════════
    local TOPBAR_H = 42   -- altura da barra superior

    local TopBar = InsertTheme(Create("Frame", MainFrame, {
        Name             = "TopBar",
        Size             = UDim2.new(1, 0, 0, TOPBAR_H),
        Position         = UDim2.new(0, 0, 0, 0),
        BackgroundColor3 = Theme["Color Panel"],
        BorderSizePixel  = 0,
        -- ✅ ClipsDescendants = true para nada vazar fora da barra
        ClipsDescendants = true,
        ZIndex           = 10,
    }), "Panel")
    -- Cantos arredondados apenas nos cantos superiores
    Make("Corner", TopBar, UDim.new(0, 12))
    -- Linha separadora na base da topbar
    InsertTheme(Create("Frame", TopBar, {
        Name             = "Divider",
    ...
    -- ── Avatar (moldura + imagem) ────────────────────────────
    local AVB = 30   -- tamanho do avatar na topbar
    local AvatarBorder = InsertTheme(Create("Frame", TopBar, {
        Name             = "AvatarBorder",
        Size             = UDim2.fromOffset(AVB, AVB),
        Position         = UDim2.new(0, 10, 0.5, 0),
        AnchorPoint      = Vector2.new(0, 0.5),
    ...
    InsertTheme(Create("TextLabel", TopBar, {
        Name             = "HubTitle",
        Size             = UDim2.fromOffset(160, TOPBAR_H),
        Position         = UDim2.new(0, AVB + 16, 0, 0),
        BackgroundTransparency = 1,
        Text             = WTitle,
        TextColor3       = Theme["Color Text"],
        Font             = Enum.Font.GothamBold,
    ...
    -- ── Badge VIP ────────────────────────────────────────────
    local VipBadge = Create("Frame", TopBar, {
        Name             = "VipBadge",
        Size             = UDim2.fromOffset(28, 13),
        Position         = UDim2.new(0, AVB + 16 + 162, 0.5, 0),
        AnchorPoint      = Vector2.new(0, 0.5),
    ...
    --    ✅ FIX: ficam dentro da TopBar, alinhados à direita,
    --       sem precisar de ZIndex separado ou container extra.
    local WIN_S = 16  -- tamanho do botão

    local function WinBtn(icon, color, offsetX)
        local btn = Create("ImageButton", TopBar, {
            Size             = UDim2.fromOffset(WIN_S, WIN_S),
    ...
    -- Fechar fica a -10px da borda direita
    -- Minimizar fica a -32px (WIN_S + 6 de gap + 10)
    local CloseButton    = WinBtn("rbxassetid://10747384394", Color3.fromRGB(190, 20, 20), -10)
    local MinimizeButton = WinBtn("rbxassetid://10734896206", Color3.fromRGB(55, 55, 65),  -(10 + WIN_S + 6))

    -- ═══════════════════════════════════════════════════════════
    --  SIDEBAR — ocupa toda a altura abaixo da topbar
    --  ✅ FIX: Position começa em Y = TOPBAR_H (não em 0)
    --          Size em Y = 1 (escala) − TOPBAR_H (offset)
    -- ═══════════════════════════════════════════════════════════
    local TabW = MyLibrary.Save.TabSize

    local Sidebar = InsertTheme(Create("Frame", MainFrame, {
        Name             = "Sidebar",
        Size             = UDim2.new(0, TabW, 1, -TOPBAR_H),
        Position         = UDim2.new(0, 0, 0, TOPBAR_H),
        BackgroundColor3 = Theme["Color Panel"],
        BorderSizePixel  = 0,
        -- ✅ ClipsDescendants = true: nada da sidebar vaza pra fora
        ClipsDescendants = true,
        ZIndex           = 2,
    }), "Panel")
    Make("Corner", Sidebar, UDim.new(0, 10))

    -- ── Sub-título / username dentro da sidebar ───────────────
    local SubTitleFrame = InsertTheme(Create("Frame", Sidebar, {
        Name             = "SubTitleFrame",
    ...
    InsertTheme(Create("TextLabel", SubTitleFrame, {
        Name             = "PlayerName",
    ...
    -- Linha separadora abaixo do sub-título
    ...
    -- ── Lista de abas (TabContainer) ─────────────────────────
    local TabContainer = InsertTheme(Create("ScrollingFrame", Sidebar, {
        Name                = "TabContainer",
        Size                = UDim2.new(1, 0, 1, -42),  -- -42 = subtítulo(38) + divider(1) + gap(3)
    ...
    -- ═══════════════════════════════════════════════════════════
    --  ÁREA DE CONTEÚDO
    --  ✅ FIX: começa em X = TabW + gap, Y = TOPBAR_H + gap
    --          altura = 100% − TOPBAR_H − margens
    -- ═══════════════════════════════════════════════════════════
    local ContentContainer = Create("Frame", MainFrame, {
        Name             = "Content",
        Size             = UDim2.new(1, -(TabW + 12), 1, -(TOPBAR_H + 10)),
        Position         = UDim2.new(0, TabW + 6, 0, TOPBAR_H + 5),
        BackgroundTransparency = 1,
        ClipsDescendants = true,
    })
    ...
    local CtrlTab = MakeDrag(Create("ImageButton", MainFrame, {
        ...
        Size             = UDim2.new(0, 10, 1, -(TOPBAR_H)),
        Position         = UDim2.new(0, TabW, 0, TOPBAR_H),
    ...
    local function ApplyResize()
        ...
        CtrlTab.Position  = UDim2.new(0, ntw, 0, TOPBAR_H)
        ...
        Sidebar.Size              = UDim2.new(0, ntw, 1, -TOPBAR_H)
        ContentContainer.Size     = UDim2.new(1, -(ntw+12), 1, -(TOPBAR_H+10))
        ContentContainer.Position = UDim2.new(0, ntw+6, 0, TOPBAR_H+5)
        CtrlTab.Position          = UDim2.new(0, ntw, 0, TOPBAR_H)
    ...
    function Window:MinimizeBtn()
        ...
        if Minimized then
            ...
            -- Minimiza apenas para a altura da topbar
            Tween(MainFrame, "Size", UDim2.fromOffset(MainFrame.Size.X.Offset, TOPBAR_H), 0.28, true)
    ...
    function Window:Dialog(Configs)
        ...
        function Dialog:Button(cfg)
            ...
            b.MouseEnter:Connect(function() Tween(b,"BackgroundColor3",Theme["Color ThemeHover"],0.12) end)
            b.MouseLeave:Connect(function() Tween(b,"BackgroundColor3",Theme["Color Theme"],0.12) end)
    ...
    function Window:MakeTab(paste, Configs)
        ...
        function Tab:Visible(b)
            ...
            if not b then Container.Parent = nil
            else Container.Parent = ContentContainer end
        end
        ...
        function Tab:AddToggle(cfg)
            ...
            local TRACK_W, TRACK_H, KNOB_S = 38, 20, 14
            ...
            local locked, current = false, default
        ...
        function Tab:AddDropdown(cfg)
            ...
            local function CalcSize()
                ...
                if dropOpen then Tween(DropFrame, "Size", UDim2.fromOffset(142, scrollSz), 0.18) end
            end

            local function CloseDropdown()
                dropOpen = false; ACF.Visible = false
            ...
            local function ToggleDropdown()
                ...
                if dropOpen then CloseDropdown()
                    ...
                    dropOpen = true; ACF.Visible = true
            ...
            local function UpdateLabel()
                ...
                else SelLabel.Text = tostring(Selected or "...") end
            end
            ...
            local function AddOption(idx, val)
                ...
                local ob = Create("TextButton", DropScroll, {
                    ...
                    BorderSizePixel  = 0, ZIndex = 103, Name = "Option",
                })
                ...
                local oLabel = InsertTheme(Create("TextLabel", ob, {
                    ...
                    Text             = nm, TextColor3 = Theme["Color Text"],
                    Font             = Enum.Font.GothamMedium, TextSize = 10,
                    TextXAlignment   = Enum.TextXAlignment.Left,
                    BackgroundTransparency = 1, TextTransparency = 0.4, ZIndex = 104,
                }), "Text")
                ob.MouseEnter:Connect(function() Tween(ob,"BackgroundColor3",Theme["Color Panel"],0.1) end)
                ob.MouseLeave:Connect(function() Tween(ob,"BackgroundColor3",Theme["Color Card"],0.1) end)
                OptionMap[nm] = { btn=ob, dot=dot, lbl=oLabel, val=val, active=false }
                local function SelectThis()
                    if multi then OptionMap[nm].active = not OptionMap[nm].active; Selected[nm] = OptionMap[nm].active
        ...
        function Tab:AddAntiTools(cfg)
            ...
            Connection.FlagsChanged:Connect(function(f, v)
                ...
                if v then StartListening() else StopListening() end
            end)

            Player.CharacterAdded:Connect(function()
                if GetFlag(flag) then StopListening(); task.wait(0.5); StartListening() end
            end)
        ...
        function Tab:AddDiscordInvite(cfg)
            ...
            InsertTheme(Create("TextLabel", card, {
                ...
                Text             = title, TextColor3 = Theme["Color Text"],
            ...
            InsertTheme(Create("TextLabel", card, {
                ...
                Text             = desc, TextColor3 = Theme["Color Dark Text"],
            ...
            Create("TextLabel", holder, {
                Size             = UDim2.new(1, 0, 0, 12), Position = UDim2.new(0, 4, 0, 0),
                Text             = invite, TextColor3 = Color3.fromRGB(80, 160, 255),
                Font             = Enum.Font.Gotham, TextSize = 9,
            ...
            joinBtn.Activated:Connect(function()
                ...
                delay = true
                SetProps(joinBtn, { Text = "✓ Link Copiado!", BackgroundColor3 = Color3.fromRGB(40, 160, 80) })
                task.wait(4)
                SetProps(joinBtn, { Text = "Entrar no Servidor", BackgroundColor3 = Color3.fromRGB(30, 130, 230) })
                delay = false
            end)
            local DI = {}
